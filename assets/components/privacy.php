<!-- COOKIES START -->
<div
  class="privacy-container"
  role="dialog"
  aria-labelledby="cookieBannerTitle"
  aria-describedby="cookieBannerDescription"
>
  <div class="title-box">
    <a
      href="https://clients.slavic.media/policy/cookies/"
      title="Cookie Policy"
      aria-label="Link to Cookie Policy"
    >
      <i class="fa-solid fa-cookie-bite"></i
    ></a>
    <span id="cookieBannerTitle">
      <span class="highlited">Cookies</span> Consent
    </span>
  </div>
  <div class="info" id="cookieBannerDescription">
    <p>
      This site uses cookies to offer you a better experience and analyze site
      traffic. If you continue to use this side, you consent to our use of
      cookies.
    </p>
  </div>
  <div class="buttons">
    <button class="button cta" id="acceptBtn" aria-label="Accept cookies">
      I understand
    </button>
    <a
      href="https://clients.slavic.media/policy/cookies/"
      title="Cookie Policy"
      class="cta"
      aria-label="Learn more about cookies"
    >
      Learn more<i class="fa-solid fa-arrow-right"></i
    ></a>
  </div>
</div>
<!-- COOKIES END -->
